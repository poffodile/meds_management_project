...
class StaffReportIncidentService
{
    ...
    public function list(array $filters = [])
    {
        ...
        return $query->with(['incidentType:id,type','clients:id,name'])->where('user_id',$filters['user_id'])->latest('id')->paginate(10);
    }
    public function report_details($id){
        return StaffReportIncidents::with(['incidentType:id,type','clients:id,name'])->find($id);
    }
}
