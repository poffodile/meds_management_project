...
const ENDPOINT = '/frontend2/medication-round-split-b';
...
// Quick-action links that replace the old profile card in the right rail.
const QUICK_ACTIONS = [
...
export default function MedicationRoundSplitB({ rounds = [], grid = {}, date, currentRound = 'morning', closures = {}, home }) {
    const isMobile = useMediaQuery('(max-width: 768px)');
    const isSm = useMediaQuery('(max-width: 576px)');
    // The rail drops below the list (via flex-wrap) well before 768px, so give it its own breakpoint:
    // whenever it's below, render it full-width + un-zoomed instead of the tight right-aligned side rail.
    ...
    return (
        <AppShell title="Medication round">
            <Head title="Medication round · split B" />
            <Box px={{ base: 0, sm: 10 }} pb={14}>
                ...
                {/* Main + rail */}
                <Box style={{ display: 'flex', flexWrap: 'wrap', gap: 40, alignItems: 'flex-start' }}>
                    ...
                    {/* Right rail — redesigned: Quick actions · stat trio · Today's outcomes.
                        The whole component is scaled down uniformly with `zoom`: every box AND its contents
                        (text, numbers, donut, gaps) shrink together in the same proportion. `flex: 0 1 auto`
                        lets the rail reflow tight around the shrunk boxes so the resident list fills the space. */}
                    <Stack gap={22} align={railBelow ? (isSm ? 'stretch' : 'center') : 'flex-end'} style={{ flex: railBelow ? '1 1 100%' : '0 1 auto', minWidth: 0, maxWidth: railBelow ? undefined : 420, paddingRight: railBelow ? 0 : 14, zoom: railBelow ? undefined : 0.9 }}>
                        {/* Quick actions (replaces the profile card in the mockup) */}
                        <Box style={{ ...card, width: '100%', maxWidth: railBelow ? (isSm ? undefined : 340) : 290, padding: isSm ? '18px 16px' : '22px 24px' }}>
                            <Text fz={16} fw={800} c={TXT} mb={14}>Quick actions</Text>
                            {/* 4 items in a single row — minmax(0,1fr) so the squares shrink to equal quarters
                                and stay inside the box (plain 1fr overflows the right padding when narrow). */}
                        ...
                        {/* Stat trio */}
                        <Group gap={12} wrap="nowrap" align="stretch" style={{ width: '100%', maxWidth: railBelow ? (isSm ? undefined : 340) : 290 }}>
                            {[{ l: 'Given', v: roundDone, c: GREEN }, { l: 'Flagged', v: flaggedCount, c: ORANGE }, { l: 'Left', v: Math.max(roundTotal - roundDone, 0), c: TXT }].map((s) => (
                                <Box key={s.l} style={{ ...card, flex: 1, minWidth: 0, padding: '16px 8px', textAlign: 'center' }}>
                                    <Text fz={26} fw={800} c={s.c} lh={1}>{s.v}</Text>
                                    <Text fz={12} c="dimmed" mt={5}>{s.l}</Text>
                                </Box>
                            ))}
                        </Group>
                        ...
                        <Box style={{ ...card, width: '100%', maxWidth: railBelow ? (isSm ? undefined : 340) : 290, padding: isSm ? '18px 16px' : '22px 24px' }}>
                            <Group justify="space-between" align="center" mb={4} wrap="nowrap">
                                <Text fz={16} fw={800} c={TXT}>Today's outcomes</Text>
                                <Box style={{ padding: '4px 12px', borderRadius: 999, whiteSpace: 'nowrap', fontSize: 12.5, fontWeight: 700, background: onTrack ? 'light-dark(#E7F3E8, rgba(31,138,91,0.20))' : 'light-dark(#FCEBDD, rgba(232,132,43,0.22))', color: onTrack ? '#1F8A5B' : '#B5601A' }}>{onTrack ? 'On track' : 'Behind'}</Box>
                            </Group>
                            <Group justify="center" my={10}>
                                <RingProgress size={168} thickness={16} roundCaps
                                    ...
                                    label={<Box ta="center"><Text fz={11} c="dimmed" lh={1}>Doses</Text><Text fz={30} fw={800} c={TXT} lh={1.2}>{outc.total}</Text></Box>} />
                            </Group>
                            <Stack gap={11} mt={4}>
                                {[{ l: 'Given', c: GREEN, v: outc.given }, { l: 'Due', c: GRAY, v: outc.due }, { l: 'Refused', c: REFUSED, v: outc.refused }, { l: 'Missed', c: MISSED, v: outc.missed }].map((s) => (
                                    <Group key={s.l} justify="space-between" wrap="nowrap">
                                        <Group gap={10} wrap="nowrap"><Box w={11} h={11} style={{ borderRadius: '50%', background: s.c, flexShrink: 0 }} /><Text fz={13.5} fw={600} c={TXT}>{s.l}</Text></Group>
                                        <Text fz={13.5} fw={700} c="#98A1AB">{opct(s.v)}%</Text>
                                    </Group>
                                ))}
                            </Stack>
                        </Box>
