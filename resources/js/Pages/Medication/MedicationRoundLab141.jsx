...
const ENDPOINT = '/medication/medication-round-lab1-4-1';
...
export default function MedicationRoundLab141({ rounds = [], grid = {}, date, currentRound = 'morning' }) {
    ...
    // ---------------------------------------------------------------------------
    // OPTION 2 — Header Banner (Apple Health). Very compact: name + close, room on
    // its own line, age•weight•round, then the chips stacked vertically beneath.
    const ageWeightRound = selected ? [
    ...
    const residentHeader = selected && (
        <Box style={surfacePrimary}>
            <Group align="flex-start" gap="md" wrap="nowrap" p="lg">
                <Avatar src={selected.photo || undefined} color={avatarColor(selected.name ?? '')} radius="lg" size={60}>
                    {initials(selected.name ?? '')}
                </Avatar>
                <Box style={{ flex: 1, minWidth: 0 }}>
                    {/* Name (strongest) + close */}
                    <Group justify="space-between" align="flex-start" wrap="nowrap" gap="sm">
                        <Text fz={22} fw={800} c="gray.9" lh={1.15} truncate>{selected.name}</Text>
                        <ActionIcon variant="subtle" color="gray" radius="md" onClick={() => setSelectedId(null)} title="Close" style={{ flexShrink: 0 }}>
                            <IconX size={18} />
                        </ActionIcon>
                    </Group>
                    {selected.room && <Text fz="sm" fw={600} c="gray.6" mt={2}>Room {selected.room}</Text>}
                    <Text fz="sm" c="gray.6" mt={2}>{ageWeightRound}</Text>
                    {/* Chips stacked vertically — compact banner */}
                    <Stack gap={6} mt={10} align="flex-start">
                        <InfoChip icon={allergies.length ? IconAlertTriangle : undefined}
                            color={allergies.length ? 'red' : 'gray'}
                            label={allergies.length ? `Allergy: ${allergies.join(', ')}` : 'Allergy None'} />
                ...
                <Group gap="sm" wrap="nowrap" align="center">
                    ...
                    <Box>
                        <Group gap={8} align="center">
                            ...
                            <Badge color="grape" variant="light" radius="sm" size="sm">Lab 1.4.1</Badge>
                        </Group>
    ...
    return (
        <>
            <Head title="Medication Round (Lab 1.4.1)" />
...
MedicationRoundLab141.layout = (page) => <AppShell>{page}</AppShell>;
