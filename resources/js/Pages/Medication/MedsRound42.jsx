    ...
    return (
        <Group gap="sm" wrap="nowrap" align="center" py={9} style={{ borderTop: `1px solid ${LINE}` }}>
            ...
            <Box style={{ flex: 1, minWidth: 0 }}>
                ...
                {recorded && (row.reason || row.notes || row.witnessed_by) && (
                    <Text fz="xs" c={tone.c} mt={3} lh={1.3}>
                        {[row.reason && `Reason: ${row.reason}`, row.notes && `Note: ${row.notes}`, row.witnessed_by && `Witness: ${row.witnessed_by}`].filter(Boolean).join('  ·  ')}
                    </Text>
                )}
            </Box>
            {recorded ? (
...
/** Derived sort metrics for a resident (scheduled total, done, % complete, status urgency rank). */
function residentSortMetrics(resident) {
    ...
    // Higher rank = more attention needed (overdue → due → in progress → complete → no meds).
    const statusRank = total === 0 ? 0 : overdue > 0 ? 4 : done === total ? 1 : done > 0 ? 2 : 3;
...
export default function MedsRound42({ rounds = [], grid = {}, date, currentRound = 'morning', closures = {}, home }) {
    ...
    // Sortable residents table — defaults to Name A→Z (matches the backend ordering).
    const [sort, setSort] = useState({ key: 'name', dir: 'asc' });
    const toggleSort = (key) => setSort((s) => (s.key === key
        ? { key, dir: s.dir === 'asc' ? 'desc' : 'asc' }
        : { key, dir: key === 'name' ? 'asc' : 'desc' }));
    const sortKeyVal = (r) => {
        const m = residentSortMetrics(r);
        if (sort.key === 'due') return m.total;
        if (sort.key === 'progress') return m.pct;
        if (sort.key === 'status') return m.statusRank;
        return (r.name || '').toLowerCase(); // name
    };
    const sorted = [...filtered].sort((a, b) => {
        const c = String(sortKeyVal(a)).localeCompare(String(sortKeyVal(b)), undefined, { numeric: true });
        const tie = c !== 0 ? c : (a.name || '').localeCompare(b.name || '');
    ...
    return (
        <AppShell>
            ...
            <Box style={{ background: CREAM, borderRadius: 28, padding: isMobile ? 16 : 28 }}>
                ...
                <Box mb="lg">
                    {home && (
                        <Box style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '5px 13px', borderRadius: 999, background: '#fff', border: `1px solid ${LINE}`, marginBottom: 10 }}>
                            <IconBedFilled size={14} color="var(--mantine-color-brandTeal-6)" />
                            <Text fz="xs" fw={700} c={INK}>{home}</Text>
                        </Box>
                    )}
                    ...
                    <Box style={card({ flex: '0 1 800px', minWidth: 0, marginLeft: isMobile ? undefined : 28, order: isMobile ? 1 : 2 })}>
                        ...
                        {!isMobile && (
                            <Group gap="md" wrap="nowrap" px="md" pb={6}>
                                <SortHeader label="Resident" k="name" sort={sort} onSort={toggleSort} style={{ flex: '2 1 210px' }} />
                                <SortHeader label="Due" k="due" sort={sort} onSort={toggleSort} style={{ flex: '1 1 90px' }} visibleFrom="md" />
                                <SortHeader label="Progress" k="progress" sort={sort} onSort={toggleSort} style={{ flex: '1 1 150px' }} visibleFrom="sm" />
                                <SortHeader label="Status" k="status" sort={sort} onSort={toggleSort} style={{ width: 78 }} />
                                <Box style={{ width: 28 }} />
                            </Group>
                        )}
                        <Box className="mr4-scroll" style={{ maxHeight: isMobile ? undefined : 520, overflowY: 'auto', overflowX: 'hidden' }}>
                            {sorted.length === 0
                                ? <Text fz="sm" c="dimmed" ta="center" py="xl">No residents with medications in this round.</Text>
                                : sorted.map((r) => <ResidentRow key={r.client_id} resident={r} expanded={expandedId === r.client_id}
