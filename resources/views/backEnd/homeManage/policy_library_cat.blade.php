...
@section('title', ' Policy Library Category')
@section('content')
                                ...
                                <div class="row">
                                    <div class="col-lg-6">
                                        <div id="editable-sample_length" class="dataTables_length">
                                            <form method='post' action="{{ url('admin/policylibrarycategory') }}"
                                    ...
                                    <div class="col-lg-6">
                                        <form method='get' action="{{ url('admin/policylibrarycategory') }}">
                                            ...
                                            @forelse($incidentType as $key=>$label)
                                                <tr>
                                                    ...
                                                    <td>
                                                        ...
                                                        |
                                                        <a href="{{ url('admin/policylibrarycategory/delete/' . $label->id) }}"
                        ...
                        <div class="modal-body">
                            <form id="incidentTypeForm">
                                <div class="form-group">
                                    ...
                                    <label class="">Category<span class="radStar ">*</span></label>
                                    <input type="text" name="type" class="form-control checkVali"
                                        placeholder="Enter Category" id="type">
                                </div>
                                <div class="form-group">
    ...
    <script>
        function status_change(id, status) {
            ...
            $.ajax({
                type: "POST",
                url: "{{ url('admin/policylibrarycategory/status-change') }}",
                data: {
                    id: id,
                    status: status,
                    _token: token
        ...
        $(document).on('click', '.openIncidentTypeModal', function() {
            ...
            if (formType === 'add') {
                $("#incidentTypeTitle").text("Add Category");
                $("#incidentTypeForm")[0].reset();
            } else if (formType === 'edit') {
                $("#incidentTypeTitle").text("Edit Category");
        ...
        $(document).on('click', '#saveChanges', function() {
            ...
            var url = "{{ url('admin/policylibrarycategory/add/') }}";
            if (incidentType_id) {
                url = "{{ url('admin/policylibrarycategory/edit/') }}";
            }
            var token = '<?php echo csrf_token(); ?>'
            var error = false;
