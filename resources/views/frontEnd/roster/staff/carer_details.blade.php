    ...
    <div class="container-fluid">
        ...
        <div class="calendarTabs leaveRequesttabs employeeDetailsTabs  m-t-20">
            <div class="tabs p-1 ">
                ...
                <!-- <button class="tab" data-tab="clockInClockOutTab">Clock IN/Clock OUT</button> -->
            </div>
            ...
            <div class="tab-content carertabcontent">
                ...
                <!-- <div class="content" id="clockInClockOutTab">
                    <div class="leave-card">
                        <div class="workHoursHeader">
                            <div class="title">Clock IN / Clock OUT</div>
                            <div class="actions">
                                <button class="allbuttonDarkClr openClockInClockOutModal" data-id="{{ $staffDetails->id }}"> <i class='bx bx-file-detail'></i> Add Clock IN / Clock OUT</button>
                            </div>
                        </div>
                        <div class="addNoteContentList">
                            @forelse ($user_logs as $date => $logs)
                            <div class="dateGroup" style="margin-bottom: 20px;">
                                <h5 style="background: #f1f5f9; padding: 10px; border-radius: 5px;">{{ \Carbon\Carbon::parse($date)->format('M d, Y') }}</h5>
                                @foreach($logs as $log)
                                <div class="certifiedList planActions remove-note" style="margin-bottom: 10px;">
                                    <span class="noteDateAndText">
                                        <div>
                                            <strong>Clock In Time :</strong> {{ !empty($log['check_in_time']) ? \Carbon\Carbon::parse($log['check_in_time'])->format('h:i A') : 'N/A' }}
                                            <strong>Clock Out Time :</strong> {{ !empty($log['check_out_time']) ? \Carbon\Carbon::parse($log['check_out_time'])->format('h:i A') : 'N/A' }}
                                        </div>
                                    </span>
                                    <button class="danger delete-log" data-id="{{ $log['id'] }}"><i class="bx  bx-trash"></i> </button>
                                </div>
                                @endforeach
                            </div>
                            @empty
                            <p>No Clock IN / Clock OUT recorded</p>
                            @endforelse
                        </div>
                    </div>
                </div> -->
