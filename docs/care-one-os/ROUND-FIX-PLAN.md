...
**✅ Round overview + resume + early-record guard + end-round confirm complete (2026-07-23)** — built per agent advice; overview is read-only, "Start" resumes to the first outstanding resident, future-time doses confirm before recording, End round checks outstanding doses first (manager-only reopen).

**✅ Witnessed CD register complete (2026-07-23)** — the compliance review's #1-named gap. `controlled_drug_register` (schema already existed, was empty) is now written as an **append-only running-balance ledger** via `ControlledDrugRegister::record()`, which computes the balance **server-side** (last entry's balance, or the opening stock on the first movement — never a client-supplied figure) under a row lock. **Administering a CD on the round now writes a witnessed entry automatically**, carrying the witness captured at administration and recorded BEFORE the stock deduction so the opening balance is correct. Manual movements (received / disposed / returned / recount) are witnessed and recorded from the CD page, which shows each medicine's balance and its full ledger. Entries are appended, never edited — a mistake is corrected with a recount. Unwitnessed movements (e.g. supported living) are recorded as "Not witnessed", never left blank. **Note the force class (STD-07):** the register is an MDR 2001 reg 19 duty for care homes; the administration witness is CQC/NICE good practice, not statute — this is engineering support for that record-keeping, not a compliance determination. Verified: `ControlledDrugRegisterTest` (opening balance = stock, chain integrity, adjustment sets absolute, round administration writes a witnessed entry, unwitnessed recorded as such).

**Still open:** CR-04 full code-set (9-code proposal) awaits the pharmacist. **No Criticals remain open in the round write path.** CR-07 (silent write failure, 18 wrappers), CR-06 (home switcher), CR-08 (UNIQUE index — needs the append-only rework first), IM-06 (zero-stock block vs warn — registered manager). Items marked **[OWNER]** need the owner's decision; **[CLINICAL]** need a pharmacist / CSO / registered manager.
